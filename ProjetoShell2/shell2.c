...
void shell(int vetor[], int tamanho, int *comp, int *trocas) {
	...
	// Enquanto intervalo != 1, continua dividindo e ordenando o vetor
	while (intervalo != 1) {
		h = intervalo;
...
int main(int argc, char *argv[]) {
	...
	int comp_cresc = 0;
	int troca_cresc = 0;
	shell(vetor_cresc, 10000, &comp_cresc, &troca_cresc);
	
	// Shell Vetor Aleatório
	int comp = 0;  
	int troca = 0;
	shell(vetor, 10000, &comp, &troca);
	
	// Shell Vetor Decrescente
	int comp_decresc = 0;
	int troca_decresc = 0;
	shell(vetor_decresc, 10000, &comp_decresc, &troca_decresc);
